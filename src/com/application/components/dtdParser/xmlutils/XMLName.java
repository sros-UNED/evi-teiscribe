...
package com.application.components.dtdParser.xmlutils;
 ...
 *
 * <p>
 * This class contains the local, qualified, and universal forms of a name, as well as the namespace prefix and URI. The local form of a name is the unprefixed
 * name. The qualified form is constructed from the prefix, a colon, and the local name. The universal form is constructed from the namespace URI, a caret (^),
 * and the local name. If the name does not belong to a namespace, then all three forms are the same and the prefix and URI are null. Note that an empty string
 * ("") is used as the prefix for the default namespace. <b>IMPORTANT:</b> Unprefixed attribute names do not belong to a namespace.
 * </p>
 *
 * <p>
 * For example:
 ...
 *
 * <p>
 * XMLName objects that have a namespace URI are not required to have a prefix. However, setPrefix(String) must be called on such objects before getPrefix() or
 * getQualifiedName() can be called.
 * </p>
 *
 * <p>
...
public class XMLName implements Serializable {
    // ***********************************************************************
    // Public constants
    // ***********************************************************************

    private static final long serialVersionUID = -8422843100145355138L;

    /**
     * The character used to separate the URI from the local name.
     *
     * <p>
     * This follows the convention in John Cowan's SAX namespace filter and uses a caret (^), which is neither a valid URI character nor a valid XML name
     * character.
     * </p>
     */
    ...
    private static String COLON = ":", XML = "xml", XMLNS = "xmlns", W3CNAMESPACE = "http://www.w3.org/XML/1998/namespace";
     ...
     *            The local name.
     * @param prefix
     *            The namespace prefix. May be null. Use an empty string for the default namespace.
     *
     * @return The XMLName.
     */
	...
	if (((prefix != null) && (!prefix.toLowerCase().equals(XMLNS))) && (uri == null))
	    throw new IllegalArgumentException("If prefix is non-null, URI must not be null.");

	// Get the qualified and universal names. This also checks if the names
	// are legal.
	// Note that the qualified name is not set if the URI is non-null and
	// the prefix is null.

	qualified = ((uri != null) && (prefix == null)) ? null : getQualifiedName(prefix, localName);
     ...
     *
     * @param qualifiedName
     *            Qualified name. Not required to contain a prefix.
     * @param uris
     *            Hashtable containing prefixes as keys and namespace URIs as values. If qualifiedName does not contain a colon, this may be null. Use an empty
     *            string ("") for the prefix of the default namespace.
     * @return The XMLName.
	...
	if (colon == -1) {
	    // If namespaces are used, check for a default namespace. If one is
	    // found,
		...
		// If the prefix is xmlns, there is no namespace (by
		// definition).
     ...
     *
     * @param prefix
     *            The namespace prefix. May be null or empty.
     * @param localName
     *            The local name.
     */
     ...
     *
     * @param universalName
     *            The universal name. Not required to contain a URI.
     * @param prefixes
     *            Hashtable containing namespace URIs as keys and prefixes as values. If the universal name does not contain a caret, this may be null. Use an
     *            empty string ("") for the prefix of the default namespace.
     * @exception IllegalArgumentException
     ...
     *            Qualified name. Not required to contain a prefix.
     * @param uris
     *            Hashtable containing prefixes as keys and namespace URIs as values.
     * @exception IllegalArgumentException
     ...
     *
     * <p>
     * An object is equal to this XMLName object if: (1) it is an XMLName object and (2) it has the same URI and local name. Note that two XMLName objects are
	...
	// Return true or false depending on whether the objects have the same
	// local name.
    ...
    /**
     * Checks if the name is bad formed. It cannot be empty or null or have a {@link XMLName#COLON} or a {@link XMLName#SEPARATOR}.
     * 
     * @param localName
     *            The name to make the checks
     */
    ...
    /**
     * Checks if the prefix is bad formed. It cannot be null or have a {@link XMLName#COLON} or a {@link XMLName#SEPARATOR}.
     * 
     * @param prefix
     *            The prefix to make the checks
     */
    ...
    /**
     * Checks if the uri is bad formed. It cannot be empty or null or have a {@link XMLName#SEPARATOR}.
     * 
     * @param uri
     *            The uri to make the checks
     */
    ...
    /**
     * Checks if the universalName is bad formed. It cannot be empty or null and if it has a {@link XMLName#SEPARATOR} then the universalName is divided into an
     * uri and a localName to check them.
     * 
     * @param universalName
     *            The universalName to make the checks
     */
