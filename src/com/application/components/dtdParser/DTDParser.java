...
import com.application.components.dtdParser.utils.TokenList;
import com.application.components.dtdParser.utils.XMLMiddlewareException;
import com.application.components.dtdParser.xmlutils.XMLName;
 ...
 *
 * <p>
 * While DTDParser checks for most syntactic errors in the DTD, it does not check for all of them. (For example, it does not check if entities are well-formed.)
...
public class DTDParser implements Serializable {
    ...
    private static final long serialVersionUID = -7571823233995512523L;
    ...
    Hashtable namespaceURIs, predefinedEntities = new Hashtable(), declaredElementTypes = new Hashtable();
    TokenList dtdTokens;
    Reader reader;
    ...
    /**
     * Parse the DTD in an XML document containing an internal subset, reference to an external subset, or both.
     *
     * @param src
     *            A SAX InputSource for the XML document.
     * @param namespaceURIs
     *            A Hashtable of keyed by prefixes used in the DTD, mapping these to namespace URIs. May be null.
     ...
     */
    @SuppressWarnings("rawtypes")
    public DTD parseXMLDocument(InputSource src, Hashtable namespaceURIs) throws XMLMiddlewareException, MalformedURLException, IOException, EOFException {
	initGlobals();
     ...
     */
    @SuppressWarnings("rawtypes")
    public DTD parseExternalSubset(InputSource src, Hashtable namespaceURIs) throws XMLMiddlewareException, MalformedURLException, IOException, EOFException {
	initGlobals();
	this.namespaceURIs = namespaceURIs;
	openInputSource(src);
	...
	// BUG! We really need to do something with this -- drive
	// transformations
    ...
    void parseExternalSubset(boolean eofOK) throws XMLMiddlewareException, MalformedURLException, IOException, EOFException {
	entityState = STATE_DTD;
    ...
    void parseExternalSubsetDecl(boolean eofOK) throws XMLMiddlewareException, MalformedURLException, IOException, EOFException {
	...
	// Get the element type name and add it to the DTD. We store the name in
	// a Hashtable
	// so we can later check if the name has already been declared. Note
	// that we only
    ...
    @SuppressWarnings("unchecked")
    void getContentParticle(Group group, ElementType parent) throws XMLMiddlewareException, MalformedURLException, IOException, EOFException {
	Group childGroup;
	Reference ref;
    ...
    @SuppressWarnings("rawtypes")
    void getEnumeratedValue(Attribute attribute, boolean useNames, Hashtable enums) throws XMLMiddlewareException, MalformedURLException, IOException,
	    EOFException {
	String name;
    ...
    @SuppressWarnings("rawtypes")
    void getEnumeration(Attribute attribute, boolean useNames) throws XMLMiddlewareException, MalformedURLException, IOException, EOFException {
	Hashtable enums = new Hashtable();

	attribute.enums = new Vector<String>();
    ...
    void getGroup(Group group, ElementType parent) throws XMLMiddlewareException, MalformedURLException, IOException, EOFException {
    ...
    void getMixedContent(ElementType parent) throws XMLMiddlewareException, MalformedURLException, IOException, EOFException {
    ...
    @SuppressWarnings("unchecked")
    Reference getReference(Group group, ElementType parent, boolean mixed) throws XMLMiddlewareException, MalformedURLException, IOException, EOFException {
	ElementType child;
	Reference ref;
	...
	if (mixed) {
		...
		throwXMLMiddlewareException("The element type " + child.name.getUniversalName()
			+ " appeared more than once in the declaration of mixed content for the element type " + child.name.getUniversalName() + ".");
	}
    ...
    @SuppressWarnings("rawtypes")
    private void updateANYParents() throws XMLMiddlewareException {
	// A common problem when building a DTD object is that element types
	// with a content model of ANY do not correctly list parents and
	// children.
	// This method traverses the list of ElementTypes and, for each element
	// type with a content model of ANY, adds all other types as children
	// and
	...
	if (!parents.hasMoreElements()) {
		throw new XMLMiddlewareException("The DTD has no elements inside, so is not a valid DTD.");
	}
	...
	while (parents.hasMoreElements()) {
		...
		while (children.hasMoreElements()) {
		    ...
		    // We don't write dummy values, so that way the structure of
		    // DTD schema is correct
			    && (!child.name.getLocalName().startsWith("_DUMMY_") && (!child.name.getLocalName().equalsIgnoreCase("ANY")))) {
			throw new XMLMiddlewareException("Element type " + child.name.getUniversalName() + " is referenced in element type "
				+ parent.name.getUniversalName() + " but is never defined.");
		    }
		}
	    }
	}
	...
	while (e1.hasMoreElements()) {
		...
		if (attribute.type == Attribute.TYPE_NOTATION) {
			...
			if (!dtd.notations.containsKey(notation))
			    throw new XMLMiddlewareException("Notation " + notation + " not defined. Used by the " + attribute.name.getUniversalName()
    ...
    void throwXMLMiddlewareException(String s) throws XMLMiddlewareException {
    ...
    void discardUntil(String s) throws XMLMiddlewareException, MalformedURLException, IOException, EOFException {
    ...
    boolean isString(String s) throws XMLMiddlewareException, MalformedURLException, IOException, EOFException {
    ...
    void requireString(String s) throws XMLMiddlewareException, MalformedURLException, IOException, EOFException {
	...
	// that markup (&, <) can be ignored if included as a character
	// reference
	...
	while ((c = nextChar()) != quote) {
	    if (!isLatinLetter(c) && !isLatinDigit(c) && (c != '.') && (c != '_') && (c != '-'))
	...
	// Gets something that matches the Nmtoken production. Must be
	// non-empty.
	...
	// Gets something that matches the PubidLiteral production. May be
	// empty.
	...
	// Gets something that matches the SystemLiteral production. May be
	// empty.
	...
	case 0x03:
	    ...
	    if ((c == 0x0386) || (c == 0x038C) || (c == 0x03DA) || (c == 0x03DC) || (c == 0x03DE) || (c == 0x03E0))
		return true;
	...
	case 0x0A:
	    ...
	    if ((c == 0x0A5E) || (c == 0x0A8D) || (c == 0x0ABD) || (c == 0x0AE0))
		return true;
	...
	case 0x0E:
	    ...
	    if ((c == 0x0E30) || (c == 0x0E84) || (c == 0x0E8A) || (c == 0x0E8D) || (c == 0x0EA5) || (c == 0x0EA7) || (c == 0x0EB0) || (c == 0x0EBD))
		return true;
	...
	case 0x11:
	    ...
	    if ((c == 0x1100) || (c == 0x1109) || (c == 0x113C) || (c == 0x113E) || (c == 0x1140) || (c == 0x114C) || (c == 0x114E) || (c == 0x1150)
		    || (c == 0x1159) || (c == 0x1163) || (c == 0x1165) || (c == 0x1167) || (c == 0x1169) || (c == 0x1175) || (c == 0x119E) || (c == 0x11A8)
		    || (c == 0x11AB) || (c == 0x11BA) || (c == 0x11EB) || (c == 0x11F0) || (c == 0x11F9))
		return true;
	...
	case 0x1F:
	    ...
	    if ((c == 0x1F59) || (c == 0x1F5B) || (c == 0x1F5D) || (c == 0x1FBE))
		return true;
	...
	case 0x09:
	    ...
	    if ((c == 0x093C) || (c == 0x094D) || (c == 0x09BC) || (c == 0x09BE) || (c == 0x09BF) || (c == 0x09D7))
		return true;
	...
	case 0x0A:
	    ...
	    if ((c == 0x0A02) || (c == 0x0A3C) || (c == 0x0A3E) || (c == 0x0A3F) || (c == 0x0ABC))
		return true;
	...
	case 0x0F:
	    ...
	    if ((c == 0x0F35) || (c == 0x0F37) || (c == 0x0F39) || (c == 0x0F3E) || (c == 0x0F3F) || (c == 0x0F97) || (c == 0x0FB9))
		return true;
	...
	// may change the value of reader.
    	...
    	if (bufferPos >= bufferLen) {
			int advancedChars = 1;
			char[] temporalBuffer = new char[buffer.length];
			int temporalBufferLength = reader.read(temporalBuffer, 0, buffer.length - advancedChars);

		    if (temporalBufferLength == -1) {
		    	// If we've hit the end of the Reader, pop the Reader off the
		    	// stack and get the first character in the next Reader.
		    	popReader();
		    	return getChar();
		    } else {
		    	for (int i = 0; i < advancedChars; i++) {
					buffer[i] = buffer[buffer.length - 1 - i];
				}
				for (int i = 0; i < temporalBufferLength; i++) {
					buffer[i+advancedChars] = temporalBuffer[i];
				}
				bufferLen = temporalBufferLength + advancedChars;
		    	bufferPos = advancedChars;
		    }
		}
    	}
    	else {
	...
	// buffer position. It isn't really important what the character is.
	// This
    ...
    void openInputSource(InputSource src) throws XMLMiddlewareException, IOException
	...
	ReaderInfo(Reader reader, char[] buffer, URL url, String str, int type, int bufferPos, int bufferLen, int line, int column, boolean ignoreQuote,
